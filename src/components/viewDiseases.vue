<template>
  ...
  <thead class="bg-green-600 font-semibold text-center">
    ...
    <tr class="divide-x">
      <th class="px-4 py-2 font-semibold text-white shadow-lg">People with this Health Case</th>
  ...
  <tbody>
    <tr class="odd:bg-white even:bg-slate-50 divide-x" v-for="person in filteredPeople" :key="person.id">
  ...
  computed: {
    filteredPeople() {
      return this.deceased.filter(person =>
