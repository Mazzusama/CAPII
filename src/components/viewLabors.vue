<template>
    ...
    <div class="flex items-center m-10">
      <div class="w-80 px-6 py-2 bg-white rounded shadow-md ring-1 ring-gray-900/10 flex items-center flex-col justify-center">
      <h1 class="text-white text-lg font-bold p-4 text-center bg-primary w-full">EDIT HEALTH PROGRAM BENEFICIARY CATEGORY (H.P.B.C)</h1>
     
        <h1 class="Text-md m-2">Current HPBC name:</h1>
        <p class="font-semibold p-2 bg-blue-400 text-sm rounded-full text-center"> {{ displayLabor.labor_name }}</p>
        <div>
  ...
  <thead class="bg-green-600 font-semibold text-center">
    ...
    <tr class="divide-x">
      <th class="px-4 py-2 font-semibold text-white shadow-lg">People with this H.P.B.C</th>
  ...
  <tbody>
    <tr class="odd:bg-white even:bg-slate-200 divide-x" v-for="person in filteredPeople" :key="person.id">
  ...
  computed: {
    filteredPeople() {
      return this.LaborName.filter(person =>
        person.first_name.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
        person.middle_name.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
        person.last_name.toLowerCase().includes(this.searchQuery.toLowerCase())
      );
    },
  },
  methods: {
