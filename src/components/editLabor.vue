  ...
  <div>
    ...
    <div class="flex items-center justify-center h-screen">
      <div class="w-80 px-6 py-4 bg-white rounded shadow-md ring-1 ring-gray-900/10 flex items-center flex-col justify-center">
        <h1 class="text-white text-lg font-bold p-4 bg-primary w-full">EDIT LABOR</h1>
        <form @submit.prevent="submit">
          <h1 class="Text-md m-2">Current Labor name:</h1>
          <p class="font-semibold p-1 bg-blue-400 text-sm m-2 rounded-full text-center"> {{ displaylabor.labor_name }}</p>
          <div>
            <label class="block text-sm font-bold w-full" for="name">
              Name of Labor
            </label>
            <input
              v-model="laborName"
...
export default {
  ...
  methods: {
    ...
    submit() {
      const url = `https://ejohncarlsrizz.pythonanywhere.com/labor/${this.id}/`;
      axios.put(url, { name: this.laborName }, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem('access_token')}`
        }
